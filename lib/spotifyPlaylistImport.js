...
/**
 * Client-credentials requests have no user country. Spotify requires `market` or content is unavailable (often 403).
 * @param {string} url absolute Spotify API URL (initial or `next` link)
 * @param {string} market ISO 3166-1 alpha-2
 */
function ensurePlaylistUrlHasMarket(url, market) {
  try {
    const u = new URL(url);
...
async function fetchAllPlaylistTracks(playlistId, token, market) {
  const out = [];
  const base = `${SPOTIFY_API}/playlists/${encodeURIComponent(playlistId)}/tracks?limit=${PLAYLIST_PAGE}&offset=0&market=${encodeURIComponent(market)}`;
  let nextUrl = base;

  while (nextUrl && out.length < MAX_TRACKS) {
    const url = ensurePlaylistUrlHasMarket(nextUrl, market);
    ...
    if (!res.ok) {
      ...
      if (res.status === 403) {
        msg += ` For server-only (client credentials) access, Spotify usually requires a market (this app uses SPOTIFY_DEFAULT_MARKET=${market}, default US). Set that env var to your ISO country code on Netlify if needed. If the playlist is still blocked, Spotify may require the listener to sign in with Spotify (OAuth) for that playlist.`;
      }
      const err = new Error(msg);
      err.status = res.status;
      throw err;
...
async function importSpotifyPlaylist(urlOrId, creds) {
  ...
  const tracks = await fetchAllPlaylistTracks(playlistId, token, market);
  if (tracks.length === 0) {
